...
test("réordonne une tâche vers le haut dans la même colonne", async ({
  taskBoard,
}) => {
  const s = uid();
  ...
  await expect
    .poll(() => taskBoard.orderedTitlesAmong(A_FAIRE, all))
    .toEqual([t3, t1, t2]);

  for (const title of all) await taskBoard.archiveTask(title);
});

test("réordonne une tâche vers le bas dans la même colonne", async ({
  taskBoard,
}) => {
  const s = uid();
...
test("déplace une tâche vers une autre colonne (ajout en fin)", async ({
  taskBoard,
}) => {
  const s = uid();
  ...
  await expect
    .poll(() => taskBoard.orderedTitlesAmong(EN_COURS, [b1, a2]))
    .toEqual([b1, a2]);
  // ... et a disparu de la colonne d'origine
  await expect
    .poll(() => taskBoard.orderedTitlesAmong(A_FAIRE, [a1, a2]))
    .toEqual([a1]);
...
test("insère une tâche à une position précise dans la colonne cible", async ({
  taskBoard,
}) => {
  const s = uid();
  ...
  await expect
    .poll(() => taskBoard.orderedTitlesAmong(EN_COURS, [b1, b2, a1]))
    .toEqual([b1, a1, b2]);
  await expect
    .poll(() => taskBoard.orderedTitlesAmong(A_FAIRE, [a1]))
    .toEqual([]);
